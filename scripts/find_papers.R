find_papers <- function(
  ...
  for (file in files) {
    ...
    if (!is.null(contents$project) && (tolower(contents$project) == tolower(project))) {
      ...
      bibfile <- file.path(dirname(file), "cite.bib")

      if (file.exists(bibfile)) {

        rel_path <- make_path_relative(project_folder,
                                       dirname(file))
  ...
  if (length(res) > 0) {
    ...
    for (r in res) {
      ...
      txt <- capture.output(PrintBibliography(r$bib, .opts = list(bib.style = "alphabetic", style = "text"))) |> stringr::str_flatten(" ")

      txt <- txt |>
        stringr::str_remove_all("^\\[.+\\]\\s")

      result <- c(result, glue::glue(
        "<li><a href='{r$url}'>{txt}</a></li>"
      ))
